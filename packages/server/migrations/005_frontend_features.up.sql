...
-- 1) Normalize existing chapter positions (dedupe before creating the unique
--    index). Skipped when the unique index already exists: positions are then
--    already enforced unique, and rewriting them mid-unique-constraint could
--    transiently violate it.
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'uniq_chapters_novel_position') THEN
        WITH ranked AS (
            SELECT id,
...
CREATE TABLE IF NOT EXISTS novel_memory (
    novel_id BIGINT PRIMARY KEY REFERENCES novels(id),
...
CREATE INDEX IF NOT EXISTS idx_media_contents_deleted ON media_contents(deleted_at);
CREATE INDEX IF NOT EXISTS idx_media_contents_position ON media_contents(position);
...
CREATE UNIQUE INDEX IF NOT EXISTS uniq_chapters_novel_position ON chapters(novel_id, position) WHERE deleted_at IS NULL;
